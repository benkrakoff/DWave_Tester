...
for i in range(6, 11):
    ...
    pd.DataFrame.from_dict(H).to_csv("Gauss_Hamiltonian_{k}".format(k = i))
    ...
    sampleset.to_pandas_dataframe().to_csv("Gauss_Sampler_Data_{k}".format(k=i))

    time_data = pd.read_csv("Time_Data")
    time_data["Gauss_Hamiltonian_{k}".format(k = i)] = [t_2 - t_1]
